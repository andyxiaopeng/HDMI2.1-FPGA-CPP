...
int pp[mm + 1] = { 1, 0, 1, 1, 1, 0, 0, 0, 1 }; /* specify irreducible polynomial coeffts */  //1, 0, 1, 1, 1, 0, 0, 0, 1
...
*/
{
	...
	//
	printf("-------------------- ------------------\n");
	printf("----- 生成多项式的向量表示（十进制） ----\n");
	for (i = 0; i <= nn - kk; i++)  printf("%d ", gg[i]);
	printf("\n-------------------- ------------------\n");
	//
...
/*
 * 假设我们已经接收到以mm位符号分组的比特，表示为recd[i]，其中i=0..(nn-1)，
 * 而recd[i]是索引形式（即作为alpha的幂次）。
 * 首先，我们通过将 alpha**i 代入 rec(X) 并进行评估来计算 2*tt个综合项，
 * 并将其存储在s[i]中，其中i=1..2tt（将s[0]保留为零）。
 * 然后，我们使用Berlekamp迭代来找到错误位置多项式elp[i]。
 * 如果elp的次数大于tt，则无法纠正所有错误，因此只能输出未经纠正的信息符号。
 * 如果elp的次数小于或等于tt，则我们将alpha**i，其中i=1..n，代入elp以获得根，从而得到逆根，即错误位置的编号。
 * 如果定位到的错误数不等于elp的次数，则存在超过tt个错误，无法进行纠正。
 * 否则，我们然后解出错误位置处的错误值并进行纠正。
 * 该过程遵循Lin和Costello的方法。
 * 对于已知无法纠正的错误数较多的情况，将原样输出接收到的信息符号
 * （系统编码的优势在于希望一些信息符号能够保持良好，如果幸运的话，错误可能只出现在传输码字的校验码部分）。
 * 当然，如果需要，这些无法解决的情况可以作为错误标志返回给调用程序。
 */
{
	...
	for (i = 1; i <= nn - kk; i++)
	{
	  ...
	  /* convert syndrome from polynomial form to index form  */
		if (s[i] != 0)  syn_error = 1;        /* set flag if non-zero syndrome => error  （如果检测到的 syndrom非零，则设置错误标志） */
		s[i] = index_of[s[i]];
	};
	...
	printf("-------------------- ------------------\n");
	printf("----- 伴随矩阵S ----\n");
	for (i = 0; i <= nn - kk ; i++)  printf("%d ", s[i]);
	printf("\n-------------------- ------------------\n");
	//
...
int main()
{
	...
	//
	printf("-------------------- ------------------\n");
	printf("----- 生成多项式 幂表示（输出数值为 幂次） ----\n");
	for (i = 0; i < nn - kk + 1; ++i) printf("%d ", gg[i]); // 10 81 251 76 0
	printf("\n-------------------- ------------------\n");
	//
...
// 	gg[4] = 0;
// //	gg[5] = 0;
// 	for (i = 0; i <= nn - kk + 1; ++i) printf("%d ", gg[i]);
// 	printf("\n");
